...
namespace com.negociosit.XMPP.XMPPphoton
{
    ...
    public class XMPPClient : System.ComponentModel.INotifyPropertyChanged
    {
        ...
        public XMPPClient()
        {
            ...
            //muc suresh
            MucManager = new MucManager(this);
            lock (LogicLock)
            {
            ...
            //muc
            this.OurServiceDiscoveryFeatureList.AddFeature(new feature("http://jabber.org/protocol/muc"));
            this.OurServiceDiscoveryFeatureList.AddFeature(new feature("http://jabber.org/protocol/muc#user"));
            this.OurServiceDiscoveryFeatureList.AddFeature(new feature("http://jabber.org/protocol/muc#admin"));
            this.OurServiceDiscoveryFeatureList.AddFeature(new feature("http://jabber.org/protocol/muc#owner"));
        ...
        //suresh MUC
        public MucManager MucManager = null;
        ...
        {
            if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Ready)
            {
                try
                {
                    IQ response = GenericIQLogic.SendPing(this.Domain, true, 1000);
                    if ((response == null) && (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Ready) && (AttemptReconnectOnBadPing == true))
                    {
                        Disconnect();
        ...
        protected virtual void StateChanged()
        {
            ...
            if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.CanBind)
            {
                Ready = false;
                // We can now bind to our resource
                GenericIQLogic.Start();
            }
            else if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Bound)
            {
                if (ShouldDoSession == true)
                {
                    XMPPState = com.negociosit.XMPP.XMPPphoton.XMPPState.Sessioning;
                ...
                else
                {
                    ...
                    XMPPState = com.negociosit.XMPP.XMPPphoton.XMPPState.Ready;
                }
            }
            else if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Session)
            {
                Ready = false;
                if (RetrieveRoster == true)
                    RosterLogic.Start();
            ...
            else if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Unknown)  // We be logged out
            {
                Ready = false;
            ...
            else if (XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Ready)
            {
                Ready = true;
        ...
        protected virtual void GotRoster()
        {
           ...
           //edited by photon
            this.XMPPAccount.LastPrescence.Status = PresenceStatus.Status;
            UpdatePresence();
            //changed by photon 
            FireListChanged(1);
        }
        ...
        //suresh remove roster from list
        public void DeleteFromRoster(JID jid)
        {
            RosterLogic.DeleteFromRoster(jid);
        }
        ...
        public void Connect(ILogInterface log)
        {
            try
            {
                ...
                XMPPConnection.OnStanzaReceived += new com.negociosit.XMPP.XMPPphoton.XMPPConnection.DelegateStanza(XMPPConnection_OnStanzaReceived);
        ...
        public bool Ready
        {
            get
            {
                if (this.XMPPState == com.negociosit.XMPP.XMPPphoton.XMPPState.Ready)
                    return true;
        ...
        //added photon suresh for without receipt chat
        public void SendChatWithOutReceipt(string strMessage,JID jidTo)
        {
             TextMessage txtmsg = new TextMessage();
            txtmsg.Received = DateTime.Now;
            txtmsg.From = JID;
            txtmsg.To = jidTo;
            txtmsg.Message = strMessage;
            GenericMessageLogic.SendChatMessageWithOutReceipt(txtmsg);
        ...
        //suresh
        public delegate void DelegateOnPresence(PresenceMessage pres);
        public event DelegateOnPresence OnRosterPresence = null;
        internal void FireOnPresence(PresenceMessage pres)
        {
            if (OnRosterPresence != null)
                OnRosterPresence(pres);
        }
        public delegate void DelegateNewConversationItem(RosterItem item, bool bReceived, TextMessage msg,string messageID);
        public event DelegateNewConversationItem OnNewConversationItem = null;
        internal void FireNewConversationItem(RosterItem item, bool bReceived, TextMessage msg, string messageID)
        {
            if (OnNewConversationItem != null)
                OnNewConversationItem(item, bReceived, msg,messageID);
        }
        //suresh group chat event handler
        public delegate void DelegateGroupConversation(Message groupChat, string messageBody);
        public event DelegateGroupConversation OnNewGroupConversation = null;
        internal void FireNewGroupConversation(Message groupChat, string messageBody)
        {
            if (OnNewGroupConversation != null)
                OnNewGroupConversation(groupChat, messageBody);
        }
        //end
        //suresh vcard request
        #region start vcard Request handler
        public delegate void DelegateVcardHandler(vcard objVcard, string objFrom);
        public event DelegateVcardHandler OnVcardRequest = null;
        internal void FireDelegateVcardHandler(vcard objVcard,string objFrom)
        {
            if (OnVcardRequest != null)
                OnVcardRequest(objVcard,objFrom);
        }
        #endregion vcard handler
        //suresh message Recipt handler
        public delegate void DelegateReceiptHandler(JID fromJID, string messageID);
        public event DelegateReceiptHandler OnReceiveMessageReceipt = null;
        internal void FireDelegateReceiptHandler(JID fromJID, string messageID)
        {
            if (OnReceiveMessageReceipt != null)
                OnReceiveMessageReceipt(fromJID, messageID);
        }
        //suresh Stream management XEP-0198 handler//
        public delegate void DelegateStreamManagementHandler(string streamXML);
        public event DelegateStreamManagementHandler OnStreamManagement = null;
        internal void FireDelegateStreamManagementHandler(string streamXML)
        {
            if (OnStreamManagement != null)
                OnStreamManagement(streamXML);
        }
               ...
               else if (elem.Name == "presence")
               {
                ...
                if (msg != null)
                {
                    if (msg is IQ)
                        ...
                        OnPresence(msg as PresenceMessage);
               }
            }
            catch(Exception  ex)
            {
        ...
        protected virtual PresenceMessage  OnPresence(PresenceMessage pres)
        {
            //fire handler suresh
            FireOnPresence(pres);
        ...
        {
            System.Diagnostics.Debug.WriteLine("Send:-->" + strXML);
        ...
        {
            System.Diagnostics.Debug.WriteLine("Recev:<--" + strXML);
